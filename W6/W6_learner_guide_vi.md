...
- **MH-SEC** — Security via AWS Config (Config rule + chuỗi auto-remediation là phần lõi; KMS CMK hoặc Security Hub là path hỗ trợ)
...
**Nội dung chính**: AWS security monitoring stack (CloudTrail, GuardDuty, Config, Security Hub, Detective, Inspector), KMS và envelope encryption, Config-based incident response automation, environment audit

**Bắt buộc thực hành**: KMS lab (tạo một customer managed key, tách admin role và use role trong key policy, mã hóa ít nhất 2 application resource), Config IR course (tạo một Config rule và link một remediation action — đây là hai bước console riêng biệt, và chuỗi remediation là PHẦN LÕI của MH-SEC), Environment Audit lab (IAM Credential Report, S3 public access audit, CloudTrail verification)

**Khái niệm phải nắm (đọc + thảo luận)**: Core Security SimuLearn là một concept walkthrough, không phải lab xây gì đó — dùng nó để review chuỗi security detection (IAM least privilege, CloudTrail logging, GuardDuty findings, Config rules). Course Security Best Practices nói về cách mỗi service kết nối trong chuỗi detection.

**Cấu trúc MH-SEC — cái gì bắt buộc vs cái gì hỗ trợ**:
- **Bắt buộc**: Ít nhất một Config rule (managed rule) được deploy + chuỗi auto-remediation được wire (Config rule non-compliant → EventBridge → SSM Automation hoặc Lambda). Đây là mức tối thiểu để thỏa MH-SEC. Tên SSM Automation document phải hiển thị trong remediation configuration panel — screenshot panel đó cho Evidence Pack của bạn.
- **Supporting Path A**: KMS Customer Managed Key (CMK) với key policy tách KeyAdministrators khỏi KeyUsers, và ít nhất 2 application resource được mã hóa bằng SSE-KMS.
- **Supporting Path B**: AWS Security Hub được bật với ít nhất một security standard active (CIS AWS Foundations, AWS Foundational Security Best Practices, hoặc PCI DSS) và ít nhất một finding đã được review.
- Bạn cần phần bắt buộc CỘNG một path hỗ trợ. GuardDuty một mình không thỏa MH-SEC.
- Viết một câu trade-off 1–2 câu trong phần MH-SEC của Evidence Pack giải thích bạn chọn path hỗ trợ nào và vì sao nó hợp với security posture của project bạn.
...
- Khác biệt giữa AWS-managed key (`aws/s3`) và Customer Managed Key (CMK): AWS-managed key do AWS kiểm soát — bạn không set được key policy hay rotate theo ý. CMK là cái bạn tạo, nơi bạn quyết định ai là KeyAdministrators và KeyUsers. Với bằng chứng MH-SEC Path A, chỉ một CMK mới chứng minh được bạn thực sự kiểm soát encryption.
- Tạo một Config rule và thêm một remediation action là hai bước console riêng biệt. Sau khi tạo rule, bạn phải quay lại, click "Edit remediation action," và link SSM Automation document của bạn. Không có bước thứ hai đó, rule phát hiện vấn đề nhưng không bao giờ sửa.

**Mẹo thực hành**: Chạy toàn bộ chuỗi automated remediation end-to-end — tạo một Config rule, link một remediation action, cố ý làm một resource non-compliant, xem SSM Automation execution xuất hiện trong Systems Manager Automation history. Capture screenshot execution history đó cho Evidence Pack của bạn. Đây là cái phân biệt câu chuyện "detect" với câu chuyện "detect and fix".
...
4. **Security via Config (MH-SEC)**: Xác nhận Config rule remediation action đã được link — click vào rule và show tên SSM Automation document trong remediation panel. Screenshot nó. Xác nhận path hỗ trợ của bạn đã sẵn sàng (KMS CMK với admin/use role tách trong key policy, hoặc Security Hub với một standard active). Viết câu trade-off 1–2 câu của bạn.
...
2. **W6 Architecture (~3 phút)**: đi qua cả 4 phần bổ sung MH theo thứ tự:
   ...
   - Tầng Security (MH-SEC): Config rule + remediation action đã link (tên SSM Automation document hiển thị) + path hỗ trợ + câu trade-off 1–2 câu
...
4. **Live Demo (~3–4 phút)**: ASG auto-healing, Config rule với remediation document đã link, tagging strategy + bằng chứng cost allocation tag activation, Cost Explorer filter theo tag, Automated Cost Guard dừng một resource không tag (CloudTrail Stop event), KMS key policy hoặc Security Hub standard. Nếu bạn làm bonus: EBS volume settings (type, IOPS, throughput) và TA findings config-based với before/after
...
4. **MH-SEC — Security via AWS Config**:
   - Ít nhất một Config rule (managed rule) được deploy
   - Chuỗi auto-remediation được wire: Config rule → EventBridge → SSM Automation hoặc Lambda (tên SSM Automation document hiển thị trong remediation panel — screenshot nó)
   - Một path hỗ trợ: hoặc KMS CMK với tách admin/use trong key policy và ít nhất 2 resource mã hóa bằng SSE-KMS, HOẶC Security Hub được bật với ít nhất một standard active và một finding đã review
   - Câu trade-off 1–2 câu: bạn chọn path hỗ trợ nào và vì sao nó hợp với security posture của project bạn?
...
**Bonus (+0.25)**: Deploy một AWS Foundational Security Best Practices hoặc PCI-DSS conformance pack ở mức account. Show compliance dashboard với ít nhất một rule ở trạng thái compliant.

---

## Cách chấm điểm

- **Group Architecture (20%)**: Cả 4 phần bổ sung MH có được cấu hình đúng và đặt đúng vị trí trong diagram của bạn không? Team có defend được các lựa chọn — Automated Cost Guard quyết định dừng cái gì và vì sao role của nó là least-privilege, tagging strategy của bạn enforce cái gì, bạn chọn path MH-SEC nào và vì sao?
- **Individual QnA (30%)**: Khả năng giải thích Automated Cost Guard hoạt động end-to-end (schedule → Lambda → tag check → stop, cộng đường Budgets → SNS và vì sao trigger cost thật trễ), quy trình tagging hai bước + cost allocation tag activation, chuỗi Config auto-remediation, và KMS envelope encryption — khi bị gọi. Mọi thành viên đều có thể bị chọn.
- **Deployment and Evidence (40%)**: Phần nặng nhất. Chấm theo Evidence Pack của bạn từng mục.
  - Điểm 4 yêu cầu: tổng account ≤ $150; cả 4 mục MH có screenshot và note; tagging strategy với bằng chứng cost allocation tag activation và quan sát baseline cost breakdown; Automated Cost Guard với một resource thực sự bị scheduled Lambda least-privilege dừng (CloudTrail Stop event) cộng Budgets $150 → SNS → Lambda được demo và latency ADR; CloudWatch dashboard với metric CWAgent; alarm ở trạng thái OK; Log Insights query đã lưu; Config rule với SSM Automation remediation đã link; KMS CMK với tách admin/use HOẶC Security Hub standard được bật; slide link tới Evidence Pack
  - Điểm 5 yêu cầu tất cả những trên CỘNG một trong: auto-healing event trong CloudWatch Activity Log với timestamps, Config remediation execution hiển thị trong SSM Automation history, hoặc một Optimization Actions bonus hoàn chỉnh (TA finding config-based với before/after remediation, lý tưởng là cost impact được lượng hóa)
...
- **MH-SEC yêu cầu chuỗi Config + remediation — GuardDuty một mình không đủ.** Wire SSM Automation remediation action ngay trong session bạn tạo Config rule. Lỗi QnA phổ biến nhất tuần này là các nhóm tạo một rule, tưởng là xong, và không show được một SSM Automation execution trong history.
...
- **Mọi người cần hiểu Automated Cost Guard và chuỗi Config auto-remediation.** Đây là hai chủ đề khả năng cao nhất trong individual QnA. Luyện giải thích: (1) schedule trigger Lambda thế nào, nó quyết định dừng cái gì từ tag `keep=true` thế nào, vì sao role là least-privilege, và vì sao trigger cost thật trễ; (2) Config phát hiện non-compliance thế nào, EventBridge route event thế nào, và SSM Automation sửa resource thế nào.
...
| AWS Lambda + EventBridge Scheduler | Chạy code theo lịch; describe và stop billable compute | Lõi MH-COST-A — Automated Cost Guard dừng EC2/RDS không tag theo lịch hằng ngày, với một role least-privilege và một CloudTrail Stop event làm bằng chứng |
...
| AWS Config | Ghi lại lịch sử cấu hình resource; đánh giá compliance rule; trigger remediation | Lõi MH-SEC — phát hiện configuration drift và tự động sửa qua SSM Automation. Config rule + chuỗi remediation là deliverable MH-SEC bắt buộc. |
| AWS KMS (CMK) | Quản lý encryption key bạn tạo và kiểm soát; key policy tách admin khỏi use | Bằng chứng hỗ trợ MH-SEC Path A — mã hóa dữ liệu at rest bằng key bạn sở hữu. Chỉ một CMK (không phải AWS-managed key) mới thỏa MH-SEC Path A. |
| AWS Security Hub | Tổng hợp security finding; map tới compliance standard (CIS, PCI DSS, AWS Foundational) | Bằng chứng hỗ trợ MH-SEC Path B — show security posture của bạn theo một standard được công nhận |
