...
- **MH-SEC** — Security via AWS Config (Config rule + auto-remediation chain is the core; KMS CMK or Security Hub as supporting path)
...
**Homework (Monday evening)**: Read "Optimize Your Cloud Governance: Balance Security and Cost" — focus on Module 3 (Config for Cost) and Module 4 (Tagging). These feed directly into your MH-COST-V deliverables. Module 2 (Organizations/SCPs/RCPs) is context reading — you will NOT be delivering SCP or Tag Policy JSON this week. Module 5 (Security-Cost Balance) helps you write the 1-2 sentence trade-off justification in your MH-SEC section. Tuesday morning there is a quick discussion: "What is one cost-vs-security trade-off your group is considering?"
...
**Main topics**: AWS security monitoring stack (CloudTrail, GuardDuty, Config, Security Hub, Detective, Inspector), KMS and envelope encryption, Config-based incident response automation, environment audit

**Must-do hands-on**: KMS lab (create a customer managed key, separate admin and use roles in the key policy, encrypt at least 2 application resources), Config IR course (create a Config rule and link a remediation action — these are two separate console steps, and the remediation chain is the CORE of MH-SEC), Environment Audit lab (IAM Credential Report, S3 public access audit, CloudTrail verification)

**Must-know concepts (read + discuss)**: The Core Security SimuLearn is a concept walkthrough, not a build-something lab — use it to review the security detection chain (IAM least privilege, CloudTrail logging, GuardDuty findings, Config rules). The Security Best Practices course covers how each service connects in the detection chain.

**MH-SEC structure — what is required vs what supports it**:
- **Required**: At least one Config rule (managed rule) deployed + auto-remediation chain wired (Config rule non-compliant → EventBridge → SSM Automation or Lambda). This is the minimum to satisfy MH-SEC. The SSM Automation document name must be visible in the remediation configuration panel — screenshot that panel for your Evidence Pack.
- **Supporting Path A**: KMS Customer Managed Key (CMK) with key policy separating KeyAdministrators from KeyUsers, and at least 2 application resources encrypted with SSE-KMS.
- **Supporting Path B**: AWS Security Hub enabled with at least one security standard active (CIS AWS Foundations, AWS Foundational Security Best Practices, or PCI DSS) and at least one finding reviewed.
- You need the required part PLUS one supporting path. GuardDuty alone does not satisfy MH-SEC.
- Write a 1-2 sentence trade-off statement in your MH-SEC Evidence Pack section explaining which supporting path you chose and why it fits your project's security posture.
...
- The difference between the AWS-managed key (`aws/s3`) and a Customer Managed Key (CMK): the AWS-managed key is controlled by AWS — you cannot set a key policy or rotate it on demand. A CMK is one you create, where you decide who the KeyAdministrators and KeyUsers are. For MH-SEC Path A evidence, only a CMK demonstrates that you actually control the encryption.
- Creating a Config rule and adding a remediation action are two separate console steps. After you create the rule, you must go back, click "Edit remediation action," and link your SSM Automation document. Without that second step, the rule detects problems but never fixes them.

**Hands-on tip**: Run the full automated remediation chain end-to-end — create a Config rule, link a remediation action, make a resource non-compliant on purpose, watch the SSM Automation execution appear in Systems Manager Automation history. Capture that execution history screenshot for your Evidence Pack. This is what separates a "detect" story from a "detect and fix" story.
...
4. **Security via Config (MH-SEC)**: Confirm the Config rule remediation action is linked — click into the rule and show the SSM Automation document name in the remediation panel. Screenshot it. Confirm your supporting path is in place (KMS CMK with separate admin/use roles in key policy, or Security Hub with a standard active). Write your 1-2 sentence trade-off statement.
...
2. **W6 Architecture (~3 min)**: walk through all 4 MH additions in order:
   ...
   - Security layer (MH-SEC): Config rule + remediation action linked (SSM Automation document name visible) + supporting path + 1-2 sentence trade-off statement
...
4. **Live Demo (~3-4 min)**: ASG auto-healing, Config rule with remediation document linked, tagging strategy + cost allocation tag activation evidence, Cost Explorer filtered by tag, Automated Cost Guard stopping an untagged resource (CloudTrail Stop event), KMS key policy or Security Hub standard. If you did the bonus: EBS volume settings (type, IOPS, throughput) and config-based TA findings with before/after
...
4. **MH-SEC — Security via AWS Config**:
   - At least one Config rule (managed rule) deployed
   - Auto-remediation chain wired: Config rule → EventBridge → SSM Automation or Lambda (SSM Automation document name visible in remediation panel — screenshot it)
   - One supporting path: either KMS CMK with admin/use separation in key policy and at least 2 resources encrypted with SSE-KMS, OR Security Hub enabled with at least one standard active and one finding reviewed
   - 1-2 sentence trade-off statement: which supporting path did you choose and why does it fit your project's security posture?
...
**Bonus (+0.25)**: Deploy an AWS Foundational Security Best Practices or PCI-DSS conformance pack at account level. Show the compliance dashboard with at least one rule in compliant state.

---

## How You'll Be Evaluated

- **Group Architecture (20%)**: Were all 4 MH additions correctly configured and placed in your diagram? Can your team defend the choices — how the Automated Cost Guard decides what to stop and why its role is least-privilege, what your tagging strategy enforces, which MH-SEC path you chose and why?
- **Individual QnA (30%)**: Your ability to explain how the Automated Cost Guard works end-to-end (schedule → Lambda → tag check → stop, plus the Budgets → SNS path and why the real cost trigger lags), the two-step tagging + cost allocation tag activation process, the Config auto-remediation chain, and KMS envelope encryption — when called on. Every team member can be picked.
- **Deployment and Evidence (40%)**: The biggest slice. Graded against your Evidence Pack section by section.
  - Score of 4 requires: account total ≤ $150; all 4 MH sections with screenshots and notes; tagging strategy with cost allocation tag activation evidence and baseline cost breakdown observation; Automated Cost Guard with a resource actually stopped by the scheduled least-privilege Lambda (CloudTrail Stop event) plus Budgets $150 → SNS → Lambda demonstrated and the latency ADR; CloudWatch dashboard with CWAgent metric; alarm in OK state; Log Insights query saved; Config rule with SSM Automation remediation linked; KMS CMK with admin/use separation OR Security Hub standard enabled; slides linked to Evidence Pack
  - Score of 5 requires all of the above PLUS one of: auto-healing event in CloudWatch Activity Log with timestamps, Config remediation execution visible in SSM Automation history, or a completed Optimization Actions bonus (config-based TA finding with before/after remediation, ideally cost impact quantified)
...
- **MH-SEC requires the Config + remediation chain — GuardDuty alone is not enough.** Wire your SSM Automation remediation action the same session you create the Config rule. The most common QnA failure this week is groups who created a rule, thought it was done, and cannot show an SSM Automation execution in history.
...
- **Everyone needs to understand the Automated Cost Guard and the Config auto-remediation chain.** These are the two most likely topics in individual QnA. Practice explaining: (1) how the schedule triggers the Lambda, how it decides what to stop from the `keep=true` tag, why the role is least-privilege, and why the real cost trigger lags; (2) how Config detects non-compliance, how EventBridge routes the event, and how SSM Automation fixes the resource.
...
| AWS Lambda + EventBridge Scheduler | Run code on a schedule; describe and stop billable compute | MH-COST-A core — the Automated Cost Guard that stops untagged EC2/RDS on a daily schedule, with a least-privilege role and a CloudTrail Stop event as proof |
...
| AWS Config | Records resource configuration history; evaluates compliance rules; triggers remediation | MH-SEC core — detects configuration drift and automates fixes via SSM Automation. Config rule + remediation chain is the required MH-SEC deliverable. |
| AWS KMS (CMK) | Manages encryption keys you create and control; key policy separates admin from use | MH-SEC Path A supporting evidence — encrypts data at rest with keys you own. Only a CMK (not the AWS-managed key) satisfies MH-SEC Path A. |
| AWS Security Hub | Aggregates security findings; maps to compliance standards (CIS, PCI DSS, AWS Foundational) | MH-SEC Path B supporting evidence — shows your security posture against a recognized standard |
