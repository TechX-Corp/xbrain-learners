...
3. **Seed script** (`seed_data.py`) — load CSV files vào database. Bạn tự lo database infrastructure. Chạy script một lần là data sẵn sàng.
...
2. Start monitoring API: `pip install -r requirements.txt && uvicorn monitoring_api:app --port 8000`
3. Hit từng API endpoint thủ công. Ghi lại data nào CHỈ có từ API.
4. Chạy seed script: `python seed_data.py --db-type sqlite`
