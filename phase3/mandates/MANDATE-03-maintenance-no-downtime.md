...
## Bối cảnh
Hạ tầng cần bảo trì định kỳ ngay trong giờ có khách - thay / khởi động lại node, rollout phiên bản mới. Đây là việc thường ngày của một hệ thống production. Hiện luồng ra tiền (browse → cart → checkout) chưa chắc chịu được những việc đó mà không gây gián đoạn. BTC sẽ **chủ động drain một node hoặc rolling-restart service trong giờ vận hành** để kiểm chứng.

## Yêu cầu
1. **Không downtime khách khi bảo trì.** Khi BTC drain một node hoặc rolling-restart, luồng browse → cart → checkout phải **giữ SLO** (checkout ≥ 99%, browse/cart ≥ 99.5%, storefront p95 < 1s) - không rớt request khách.
...
## Phải nộp
- Cho mentor **một khung giờ để BTC tự drain node / rolling-restart** và **chứng kiến SLO không rớt** trong lúc đó (dashboard SLO trước / trong / sau). Mentor tự xác nhận.
